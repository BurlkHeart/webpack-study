...
module.exports = {
    ...
    module: { // 用来配置第三方loader模块的
        rules: [ // 模块的匹配规则
            ...
            { test: /\.(png|jpg|gif|jpeg|svg)$/, use: 'url-loader?limit=13205&name=[hash:5][name].[ext]' },
            //1.传参：?limit=25342-->图片大小，若图片吗的字节大于你设置的字节标准，则显示路径，
           // 否则显示base64格式的
            //2.改图片被编译成了唯一的hash值，比方DNA，一共32位，继续传参设置自己的图片名:name=[name].[ext]
            //3.为了防止图片名重复，在图片名的前面hash值10位 例 ：name=[hash: 10][name].[ext]
